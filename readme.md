...
- 战斗结束按表现评定 **1-3 星**
- 连续答对会累计到全局连对记录，达成 30 题可解锁 **心算小能手**
- 每日首次进入会记录登录，连续 7 天可解锁 **持之以恒**
...
- **激励系统** — 本地保存成就、连续登录、连续答对、商店购买与装备状态
...
└── src/
    ...
    │   ├── home.ts          # 角色创建页
    │   ├── map.ts           # 世界地图页
    │   ├── battle.ts        # 战斗页（含结算/成就更新）
    │   └── profile.ts       # 我的页（成就 + 商店 + 装扮）
    └── styles/
